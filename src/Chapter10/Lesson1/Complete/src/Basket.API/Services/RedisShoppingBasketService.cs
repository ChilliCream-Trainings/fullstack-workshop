    ...
    : IShoppingBasketService
{
    ...
    public async Task<ShoppingBasket?> GetBasketAsync(string customerId, CancellationToken ct = default)
    {
        ...
        return JsonSerializer.Deserialize(data.Span, ShoppingBasketSerializationContext.Default.ShoppingBasket);
    }

    public async Task SaveBasketAsync(ShoppingBasket basket, CancellationToken ct = default)
    {
        var json = JsonSerializer.SerializeToUtf8Bytes(basket, ShoppingBasketSerializationContext.Default.ShoppingBasket);
